...
for any_chan_no in range(1, any_chan):
    ...
    for error in error_list:
        ...
        print(vtfb, vtgfb)
        np.save(
            f'data/fid_plot_data_test/qbit{qdim}_lower_bound_rel_error{error}_rank{rank}_ansatz{an}_anychan{any_chan_no}',
            vtfb)
        np.save(
            f'data/fid_plot_data_test/qbit{qdim}_upper_bound_rel_error{error}_rank{rank}_ansatz{an}_anychan{any_chan_no}',
            vtgfb)

        if vtfb < 0.05 and vtgfb < 0.05:
            print(any_chan_no)
            np.save(
                f'data/fid_plot_data_test/qbit{qdim}_lower_bound_less_than5pe_rel_error{error}_rank{rank}_ansatz{an}_anychan{any_chan_no}',
                vtfb)
            np.save(
                f'data/fid_plot_data_test/qbit{qdim}_upper_bound_less_than5pe_rel_error{error}_rank{rank}_ansatz{an}_anychan{any_chan_no}',
                vtgfb)
